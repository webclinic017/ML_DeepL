...
# de un dia para otro: (las filas de ts son continuas pero las columnas no)
    ...
    Lookback0 = ts["Lookback0"].values
    Lookback0 = Lookback0.reshape((len(Lookback0), 1))
    scaler = scaler.fit(Lookback0)
    print('Mean: %f, StandardDeviation: %f' % (scaler.mean_, sqrt(scaler.var_)))
    # normalize the dataset and print
    Lookback0 = scaler.transform(Lookback0)
    ts["Lookback0"] = Lookback0.reshape(len(Lookback0))
#    print(ts["Lookback0"])
    # inverse transform and print
#    inversed = scaler.inverse_transform(Lookback0)
#    print(inversed)
    
    
    for i in range(1, lookback_minutes):
        ts["Lookback%s" % str(i)] = ((ts["Lookback%s" % str(i)]/ts["Lookback%s" % str(i+1)])-1.0)*100.0
        temporal = ts["Lookback%s" % str(i)].values
        temporal = temporal.reshape((len(temporal), 1))
        temporal = scaler.transform(temporal)
        ts["Lookback%s" % str(i)] = temporal.reshape(len(temporal))
# Yoe: Tiene que ser en reverso porque los Lookforwards se van modificando: 
    if lookforward_minutes > 1:
        for i in reversed(range(2, lookforward_minutes + 1)):
            ts["Lookforward%s" % str(i)] = ((ts["Lookforward%s" % str(i)]/ts["Lookforward%s" % str(i-1)])-1.0)*100.0
            temporal = ts["Lookforward%s" % str(i)].values
            temporal = temporal.reshape((len(temporal), 1))
            temporal = scaler.transform(temporal)
            ts["Lookforward%s" % str(i)] = temporal.reshape(len(temporal))

    ts["Lookforward1"] = ((ts["Lookforward1"]/ts["Close"])-1.0)*100.0
    temporal = ts["Lookforward1"].values
    temporal = temporal.reshape((len(temporal), 1))
    temporal = scaler.transform(temporal)
    ts["Lookforward1"] = temporal.reshape(len(temporal))
    print(ts)
...
#######################################################################################
    ...
    if up_down_factor is None and percent_factor is not None:
        #YOE
        factor = [[percent_factor]]
        factor = scaler.transform(factor)
        percent_factor = factor[0][0]
        up = percent_factor
                            ...
                            for nLSTM in [1]:
                                for kernel_size in [3]:
                                    for batch_size in [128]:
                                        for conv_nodes in [10]:
                                            for lstm_nodes in [1]:
                                                for learning in [0.1]:                                
                                                    name = model + "_lags%s_skip%s_nConv%s_nLSTM%s_learning%s_batch_size%s_conv_nodes%s_lstm_nodes%s_kernel_size%s" %(
                                                            lags, skip, nConv, nLSTM, learning, batch_size, conv_nodes, lstm_nodes, kernel_size)   # esta es la carpeta que contendra los pkl and png
                                                    start_date = None
