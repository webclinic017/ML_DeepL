...
class IQFeedIntradayCsvBarPriceHandler(AbstractBarPriceHandler):
    ...
    def __init__(
        ...
        init_tickers=None,
        start_date=None, end_date=None, skip=None
    ):
        """
    ...
    def _open_ticker_price_csv(self, ticker):
        ...
        self.tickers_data[ticker] = pd.read_csv(
            ticker_path,
            names=["Date", "Open", "High", "Low", # YOE adaptado para iqfeed + Motivewave format
                "Close", "Volume"
            ],
...
# YOE
        if start is None and end is None:
            return df.ix[::self.skip].iterrows()
        elif start is not None and end is None:
            return df.ix[start::self.skip].iterrows()
        elif start is None and end is not None:
            return df.ix[:end:self.skip].iterrows()
        else:
            return df.ix[start:end:self.skip].iterrows()
