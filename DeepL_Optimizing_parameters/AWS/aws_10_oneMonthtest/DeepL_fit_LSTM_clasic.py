            ...
            def on_epoch_end(self, epoch, logs={}):
                if self.epoch % self.N == 0:
                    ...
                    for i in range(len(y_pred)):
                        ...
                        if np.argmax(y_pred[i]) == 1 and np.argmax(self.y_test[i]) != 1:
                            pred_F += 1
                        if np.argmax(y_pred[i]) == 2 and np.argmax(self.y_test[i]) == 2:
                            pred_T += 1
                        if np.argmax(y_pred[i]) == 2 and np.argmax(self.y_test[i]) != 2:
                            pred_F += 1
                    ...
                    if pred_T + pred_F > 0:
                        ...
                        print("Yoe: epoch, Probabilidad tot: ", self.epoch, Pr_pos)
                        print("Yoe: epoch, Cantidad total pred: ", self.epoch, pred_T + pred_F)
                    else:
