...
class Fitting_DeepL():
    def __init__(self,
        ...
        self.X_train = ts[ts.index < end_date][["Lookback%s" % str(i) for i in range(lags)]]
        self.X_test = ts[ts.index >= end_date][["Lookback%s" % str(i) for i in range(lags)]]
...
#        model.add(Reshape(input_shape=(dim_row, dim_col), target_shape=(dim_row, dim_col, 1)))
        ...
        if val_loss:
            callbacks.append(EarlyStopping(
                ...
                patience=5))
        if second_opinion:
            callbacks.append(SecondOpinion(model, data_test, clas_test, 5))
